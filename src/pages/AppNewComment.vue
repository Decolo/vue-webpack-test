  ...
  export default {
    ...
    methods:{
      ...
      fetchProducts:function(){
        ...
        xhttp.get('/commentsById',
                 {ids:ids.join(',') ,date:mytool.dateformat(vm.filterDate[vm.activeDateIdx],'yyyy-MM-dd')},
                function(res) { 
                   vm.listData={all:{products:ids.map(function(i){return {app_id:i,new_comment:0}})}};
                   for(var key in res){
                    vm.listData[key]={products:res[key].map(function(r,i){
                      if(vm.listData.all.products[i]){
                        vm.listData.all.products[i].new_comment+=r.commentNum;
                      }
                      r.new_comment=r.commentNum;
                      return r;
                    })};
                   };  
                });
              ...
              if(vm.otherProOptKey==S_ALLPRODUCT){
                yAxis=mytool.sum(vm.otherProducts,function(otpro){
                  if(!productsMap[otpro.key]){ 
                    ...
                    return productsMap[otpro.key].new_comment
                  }
                });
              }else{  
                yAxis=productsMap[vm.otherProOptKey].new_comment;
              }
              serieData.push(yAxis);
              if(vm.ownProOptKey >=0){
                ownserieData.push(productsMap[vm.ownProOptKey].new_comment)
              }
            });  
      ...
      downloadOf:function(oneday,pro){  
        if(oneday && oneday.products){
          ...
          if(!productsMap[pro.key]){ 
            ...
            let result=productsMap[pro.key].new_comment; 
            return result;
          }
        }else{
          return 0;
