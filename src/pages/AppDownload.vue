      ...
      <slide-up  class="chartArea" :sildeUp="showChart"> 
        <div class="flex filterbar">
          ...
          <div class="filter-data align-right" v-show="showProductDetail">
            <span class="fd-number">{{currtProduct.totalDownload}}</span>
            <span class="fd-text">总下载量</span>
          </div> 
        ...
        <div class="dataArea" v-if="!showProductDetail">
            ...
            <div class="datarows">
              <div class="datarow pdrow" v-for="row in allProducts" v-on:click="doShowProductDetail(row)">
                ...
                <span class="text-r"> {{row[filterSourceKey]  || 0}}</span> 
              </div>
            </div>
        </div>
        <div class="dataArea" v-if="showProductDetail">
            ...
            <div class="datarows ">
              <div class="datarow pdrow" v-for="(channel,index) in channels" >
                ...
                <span class="text-r"> {{currtProduct[channel.key] || 0}}</span> 
              </div>
            </div>
        </div>
  </main-layout>
...
<script> 
  ...
  const S_ALLCHANNEL='totalDownload'; 
      ...
      _setProductDetails:function(products, data) {
             ...
             for (let key in data) {  
                 ...
                 for (let k = 0; k < platform.length; k++) {
                     ...
                     app.downloadTimes=parseInt(app.downloadTimes);
                     product[key]= app.downloadTimes;
                     product.totalDownload += parseInt(app.downloadTimes); 
                 } 
             } 
