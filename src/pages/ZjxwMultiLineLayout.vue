      ...
      <div class="dataArea" v-show="dateIds.length>0">
            ...
            <div class="datarows">
              <div class="datarow pdrow" v-for="(row,i) in datarows" v-if="i>0"> 
                <v-clamp class="text-l fullname">{{row.title}}</v-clamp>
              ...
              <div class="newsitem" v-for="item in itemList" @click="switchItemCheck(item)">
                  <div class="flex newsinfo">
                    ...
                    <div class="flex flex-reverse">
                      <i class="checkbox" :class="{'checked':item.checked,'disabled':dateIds.length>=10 && !item.checked }">
    ...
    methods:{
      loadItems:function () {
        ...
        xhttp.get(vm.listurl,
                 vm.pageParams,
                function(res) { 
                   vm.itemList=vm.itemList.concat(res.data.map(function(i){i.id=i[vm.idKey];i.checked=false;return i})); 
      ...
      fetchData:function () {
        var vm=this; 
        var params={dataType:vm.prokey,days:30};
        params[vm.idkeys]=vm.dateIds.join(',');
