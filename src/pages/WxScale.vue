<template>
  <main-layout > 
      <silde-up  class="chartArea" :sildeUp="showChart" > 
        ...
        <swiper :on-switch="switchfilterItemIndex" :style="{width:width+'px'}" ref="chartswiper">
        ...
        <div class="dataArea">
            ...
            <div class="datafilterbar" v-show="!showChart">
               <span v-for="(filteritem,fidx) in filterkeys" :class="{active:filteritem.key === key}" v-on:click="switchfilterItem(filteritem,fidx)">{{filteritem.shorttext}}</span>
            </div>
...
<script> 
  ...
  export default {
  	data:function (){ 
        ...
        key:'history_new',
        filterkeys:[
           {key:'history_new',text:'历史总用户',shorttext:'历史总用户',options:{inited:false}},
           {key:'thirty_new',text:'近30日新增用户',shorttext:'近30日',options:{inited:false}},
           {key:'seven_new',text:'近7日新增用户',shorttext:'近7日',options:{inited:false}},
           {key:'yesterday_new',text:'昨日新增用户',shorttext:'昨日',options:{inited:false}}
        ],
        arrowStyle:{},
        lineStyle:{},
        colors:config.color,
    ...
    methods:{
      fetchData:function () {
        var vm=this;
        var today = mytool.dateformat(mytool.getDate(0), "yyyy-MM-dd");
        var yesterday = mytool.dateformat(mytool.getDate(-1), "yyyy-MM-dd");
        var pre7day = mytool.dateformat(mytool.getDate(-2), "yyyy-MM-dd");
        var pre30day = mytool.dateformat(mytool.getDate(-30), "yyyy-MM-dd");
        xhttp.get('/wxSubscriber',
                {start:pre30day ,end:today },
                function(res) {   
                  vm.chartData.history_new=res.history;
                  vm.chartData.thirty_new=res.subscriber_new;
                  vm.chartData.yesterday_new=res.yesterday_new;
                  vm.initData(['history_new','thirty_new','yesterday_new']);
                  vm.switchfilterKey('history_new');
                });
        xhttp.get('/wxSubscriber',
                {start:pre7day ,end:today },
                function(res) {    
                  vm.chartData.seven_new=res.subscriber_new;
