  ...
  <div  :style="{height:height+'px'}" class="commonChart barChart">
    <div class="tooltipsbox" v-show="showTooltip" :style="{top:tbp.top+'px',left:tbp.left+'px'}">
      ...
      <span class="tooltip" v-for="(tooltip,i ) in tooltips">
         {{tooltip.name}}:{{tooltip.data}}
      </span>
    </div>
    <svg :width="width" :height="svgHeight" ref="svg">
      <g :transform="transform(grid)">
        <g v-for="(serie,index ) in series" :key="index"> 
          <chart-rect :y="item.y" :x="item.x" :height="barwidth" :width="item.width"   v-for="(item,itemidx) in serie.rectList" :key="itemidx" :fill="itemidx | gcolor" v-on:click.native="toShowTooltip(itemidx)"></chart-rect>
        </g>
      </g>
